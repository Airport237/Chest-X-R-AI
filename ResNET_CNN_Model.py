...
# Training Loop
def train_model(model, train_loader, test_loader, transform_pipeline, device, num_epochs=5, saved_model = ""):
    if (saved_model != ""):
    ...
    for epoch in range(1, num_epochs):
        ...
        print(f"Saving Epoch {epoch} model as 'resnetInterNot.pth'")
        torch.save(model.state_dict(), "resnetInterNot.pth")


        plt.plot(epoch_losses, label='loss')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.savefig("ResNet Not Pretrained")
        ...
        print("========== Testing Epoch ==========")
        AUCs.append(test(model, test_loader, transform_pipeline, device, "resnetInterNot.pth"))
        print("AUCs So Far" +  str(AUCs))
        print("Losses So Far" + str(epoch_losses))

        plt.plot(epoch_losses, label='Testing AUC')
        plt.xlabel('Epoch')
        plt.ylabel('AUC')
        plt.savefig("Testing AUC")
